        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                if cursor.fetchone()[0] == 0:
                    print(f"Employee with ID {e_id} does not exist.")
                ...
                if not target_appointments:
                    print(f"No appointments found for employee {e_id} on {orig_day}.")
                    return 0  # No appointments found

                print(
                    f"Found {len(target_appointments)} target appointments for rescheduling."
                )
                ...
                for appointment in target_appointments:
                    ...
                    print(
                        f"\nProcessing appointment ID {a_id} (start: {start_time}, end: {end_time}, patient: {p_id})"
                    )
                    ...
                    print(f"Appointment duration: {duration}")
                    ...
                    if conflict_count > 0:
                        print(
                            f"Conflict detected for appointment ID {a_id}. Skipping rescheduling."
                        )
                    ...
                    print(
                        f"Appointment ID {a_id} rescheduled to start at {available_time.time()}."
                    )

                    # Step 6: Update the staff assignments for the appointment
                    cursor.execute(
                        """
                    ...
                    print(f"Updated ScheduledProcedureStaff for appointment ID {a_id}.")

                    # Move available_time forward by the duration of the appointment
                    available_time += duration
                    rescheduled_count += 1
                    print(f"Rescheduled {rescheduled_count} appointments so far.")

                # Commit the changes to the database
                self.connection.commit()
                print(f"Total appointments rescheduled: {rescheduled_count}")
