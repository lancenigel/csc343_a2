...
-- Step 4: Delete appointments for patients belonging to clients who are about to be deleted
DELETE FROM Appointment
WHERE p_id IN (SELECT p.p_id FROM Patient p WHERE p.c_id IN (SELECT c_id FROM ClientsToDelete));

-- Step 5: Delete pets belonging to clients who are about to be deleted
DELETE FROM Patient
WHERE c_id IN (SELECT c_id FROM ClientsToDelete);

-- Step 6: Delete the clients who have no pets or no recent appointments
DELETE FROM Client
WHERE c_id IN (SELECT c_id FROM ClientsToDelete);
